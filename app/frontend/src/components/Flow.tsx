...
export function Flow({ className = "" }: FlowProps) {
  ...
  const autoSave = useCallback(
    async (flowIdToSave?: number | null) => {
      // Use the provided flowId or fall back to current flow ID
      const targetFlowId =
        flowIdToSave !== undefined ? flowIdToSave : currentFlowId;
      ...
      autoSaveTimeoutRef.current = setTimeout(async () => {
        ...
        try {
          ...
          console.error(
            `[Auto-save] Failed to save flow ${targetFlowId}:`,
            error,
          );
        }
      ...
      if (currentFlowId) {
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            console.error(
              `[Auto-save] Failed to save new connection for flow ${flowIdAtTimeOfChange}:`,
              error,
            );
          }
        }, 100);
