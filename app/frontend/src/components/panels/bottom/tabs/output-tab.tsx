...
export function OutputTab({ className }: OutputTabProps) {
  ...
  const { getAgentNodeDataForFlow, getOutputNodeDataForFlow } =
    useNodeContext();
  const [, setUpdateTrigger] = useState(0);

  // Get current flow data
  const agentData = getAgentNodeDataForFlow(currentFlowId?.toString() || null);
  const outputData = getOutputNodeDataForFlow(
    currentFlowId?.toString() || null,
  );
  ...
  const sortedAgents = sortAgents(
    Object.entries(agentData).filter(([agentId]) => agentId !== "backtest"),
  );
